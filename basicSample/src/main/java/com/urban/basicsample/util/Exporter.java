...
public class Exporter {
    ...
    private Integer writeSubject(Integer rowNum, String subject) {
        ...
        for (Map.Entry<String, Integer> entry : names.entrySet()) {
            ...
            float res = 100 - getAttByStudent(entry.getKey()) * 100 / expAtt;
            newCell.setCellValue("Проп.: " + res + "%");
        }
    ...
    private int getAttByStudent(String name) {
        int result = 0;
        ...
        Cursor c = db.rawQuery(query, new String[]{names[0], names[1]});
        if (c.moveToFirst()) {
